...
#error "Please define a frequency band in the compiler options."
	...
	while (1)
	{
		...
		// Process Radio IRQ//根据中断源切换状态
		Radio.IrqProcess();
	}
}

void OnTxDone(void)//发送完成，lora休眠，设置状态TX
{
	Radio.Sleep();
	State = TX;
}

void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr)//接收完成，LORA休眠，将接收数据存放BUFFER,状态设为RX
{
	Radio.Sleep();
	BufferSize = size;
...
void OnTxTimeout(void)//发送时间溢出，LORA睡眠,状态=tx_timeout
{
	Radio.Sleep();
	State = TX_TIMEOUT;
}

void OnRxTimeout(void)//接收时间溢出，LORA睡眠,状态=Rx_timeout
{
	Radio.Sleep();
	State = RX_TIMEOUT;
}

void OnRxError(void)//接收错误，LORA睡眠,状态=RX_ERROR
{
	Radio.Sleep();
	State = RX_ERROR;
}
